...
local function hookfix() 
    if not InCombatLockdown() then
        if _G["ccsm_sf"] and (option("showm_sp") == true) and (UnitLevel("player") == CCS.MaxLevel and C_UnitAuras.GetPlayerAuraBySpellID(440393) == nil) and (C_MythicPlus.GetCurrentAffixes() and C_MythicPlus.GetCurrentAffixes()[1]) then
            _G["ccsm_sf"]:Show()
            _G["ccs_sf"]:Hide()             
...
local function Clicky(endstate)
    ...
    if CharacterModelScene:GetHeight() > 600 then -- This is to move model under the character equipment
        MoveModelLeft()
        if _G["ccsm_sf"] and (option("showm_sp") == true) and (UnitLevel("player") == CCS.MaxLevel) and (C_MythicPlus.GetCurrentAffixes() and C_MythicPlus.GetCurrentAffixes()[1]) then
            _G["ccsm_sf"]:Show()
        elseif _G["ccsm_sf"] then 
            _G["ccsm_sf"]:Hide() 
        end
        ...
        if factionData ~= nil then
            for _,k2 in ipairs(ks2) do  -- Reputation Bar (in the row)
                ...
                if name == "Inactive" or name == "Other" then
                    ...
                    if factionID and C_Reputation.IsFactionParagon(factionID) and k2.ParagonIcon then
...
function module:Initialize()
    ...
    if (option("hideiconborders")) then
        ...
        CharacterFrameTab1.Middle:SetPoint("TOPLEFT", CharacterFrameTab1, 0, 0)
        CharacterFrameTab1.Middle:SetPoint("TOPRIGHT", CharacterFrameTab1, 0, 0)
        ...
        CharacterFrameTab1.Middle:SetGradient("Vertical", CreateColor(.1, .1, .1, 1), CreateColor(0, 0, 0, 1)) -- Dark Gray
        ...
        CharacterFrameTab2.Middle:SetGradient("Vertical", CreateColor(.1, .1, .1, 1), CreateColor(0, 0, 0, 1)) -- Dark Gray
        ...
        CharacterFrameTab3.Middle:SetGradient("Vertical", CreateColor(.1, .1, .1, 1), CreateColor(0, 0, 0, 1)) -- Dark Gray
    ...
    ReputationFrame.ScrollBox:SetPoint("BOTTOMRIGHT", CharacterFrameInset.Bg, "BOTTOMRIGHT", 80, 0)
