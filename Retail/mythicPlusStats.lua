...
local function updatesideframe()
	...
	for x=1,8 do 
		...
		if mapspellID ~= 0 and IsSpellKnown(mapspellID) then
			...
			local spellCooldownInfo = C_Spell.GetSpellCooldown(mapspellID)
			local start = spellCooldownInfo.startTime or 0
			local duration = spellCooldownInfo.duration or 0
			local enabled = spellCooldownInfo.isEnabled or 0
			local modRate = spellCooldownInfo.modRate or 0
...
local function CreateAffixButton(index, anchorFrame, parentFrame)
    ...
    if index == 1 then
        btn:SetPoint("TOPLEFT", anchorFrame, "BOTTOMRIGHT", -168, -4)
    else
        btn:SetPoint("LEFT", _G["ccsm_aff"..(index - 1).."_btn"], "RIGHT", 10, 0)
    end
    btn:SetSize(32, 32)
...
----
	...
	btn2:SetSize(32, 32)
	btn2:SetPoint("LEFT", PaperDollSidebarTabs, "RIGHT", -5, 0)
